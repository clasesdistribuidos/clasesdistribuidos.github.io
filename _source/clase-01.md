...
*[Nota: esos mecanismos no son gratis, y la pregunta que dejan abierta —cuántas veces se ejecutó realmente lo que pedimos— tiene tres respuestas canónicas que van a organizar buena parte de la materia: **at-least-once** (al menos una vez: se reintenta hasta que pase, así que la operación puede ejecutarse varias veces y tiene que ser idempotente), **at-most-once** (a lo sumo una vez: no se reintenta, así que se ejecutó una vez o ninguna) y **exactly-once** (exactamente una vez), que es el caso ideal y, en sentido estricto, imposible. Las desarrollamos en la sección 5, ya con RPC sobre la mesa.]*

*[Nota: la clasificación viene de la literatura fundacional de RPC. La primera taxonomía sistemática está en la tesis doctoral de Bruce Jay Nelson, *Remote Procedure Call* (Carnegie Mellon, 1981; publicada también como informe de Xerox PARC, CSL-81-9), cuya sección 2.2.2 se titula justamente "Call Semantics" y enumera bastante más de tres casos: *exactly-once*, *last-one*, *last-of-many*, *at-least-once*, *crash semantics*. Un trabajo paralelo de Alfred Spector, *Performing remote operations efficiently on a local computer network* (CACM 25(4), 1982), llegaba a distinciones parecidas del lado de los mensajes. La reducción a los tres nombres que vamos a usar es la que hacen Saltzer y Kaashoek en el capítulo 4 del libro que seguimos. Vale la pena notar que Nelson perseguía *exactly-once* por la misma ambición de transparencia que produjo el Network File System, y que la implementación real que hizo después con Andrew Birrell en Xerox —*Implementing Remote Procedure Calls*, ACM TOCS 2(1), 1984— terminó ofreciendo garantías más débiles.]*
