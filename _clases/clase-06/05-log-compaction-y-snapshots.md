...
La solución tiene el carácter de una solución ad hoc: está todo el algoritmo, muy elegante, y este agregado tuvieron que incorporarlo allí también, como una optimización de último momento junto con el compaction. Tiene sentido, pero complica.
...
Es una parte compleja, y la que mayores dificultades generó entre quienes la implementaron el cuatrimestre pasado. Además desentona con la elegancia del resto del algoritmo: aquí, si ya no se puede seguir rebobinando, se envía un snapshot, el otro lo restaura, y después se le aplica el log de la manera habitual. Con eso el follower puede continuar avanzando.
