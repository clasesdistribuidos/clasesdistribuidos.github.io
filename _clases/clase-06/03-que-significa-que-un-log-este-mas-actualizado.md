...
¿Y eso implica que uno tiene más entradas por comitear que el otro? Lo que la condición garantiza es otra cosa: si se respeta, el que resulta elegido líder no puede eliminar nada que esté comiteado. El paper lo demuestra por el absurdo en la sección 5.4.3: la entrada comiteada está en una mayoría y el nuevo líder recibió votos de otra mayoría, así que al menos un nodo tiene la entrada y además lo votó; la restricción de elección hace imposible que ese líder no la tenga. El caso de F lo hace evidente: tiene entradas del término 2 y del término 3 que aparecen solamente allí, en ningún otro lugar. Si la condición fuera solamente la longitud, F —con sus once entradas— se convertiría en líder y produciría un resultado catastrófico, eliminando todo lo que ya estaba comiteado.

El resultado no se aprecia a simple vista, porque en algunos casos votan a favor y en otros en contra; la demostración general está en la sección 5.4.3 del paper. Pero hay algo muy importante que extraer de la figura: no hay un único resultado posible, no hay un único líder posible. Varios pueden ser elegidos, y según quién sea, algunas entradas se eliminan y otras no. La garantía es que nunca se va a eliminar algo que ya había sido escrito en una mayoría; pero también se pueden escribir entradas que estaban en un solo lugar.
...
Sobre este resultado hay una objeción que conviene tomar en serio, aunque no tenga respuesta cerrada. Resulta llamativo que se pierdan entradas de los servidores más adelantados: si se eligiera líder a uno de esos, prácticamente no se perderían datos y se reduciría la tasa de pérdida a largo plazo. El paper no discute exactamente esa alternativa, pero sí explica el criterio de diseño detrás (sección 5.4.1): otros algoritmos, como Viewstamped Replication, permiten elegir un líder incompleto y luego transferirle las entradas que le faltan, a costa de mecanismos adicionales. Raft prefiere que el líder ya tenga todas las entradas comiteadas desde el momento de su elección, de modo que las entradas fluyan en un solo sentido, del líder a los followers. Y hay además una consecuencia a tener en cuenta: se van a comitear entradas que nunca se le habían confirmado al cliente, porque esos 6 que estaban en tres máquinas nunca se le pudieron confirmar.
